...
@click.option("--current-ref", default="HEAD", help="Branch, tag, or commit SHA to release (defaults to HEAD)")
@click.option("--github-token", envvar="GITHUB_TOKEN", required=True, help="GitHub token (can use Actions built-in)")
@click.option("--output", default="release_notes.md", help="File to write release notes")
@click.option("--prev-ref", default="2025-06-11")
@click.option("--depth", default=1, show_default=True, help="How many levels of git dependencies to scan recursively")
def draft_release_notes(repo_url: str, current_ref: str, github_token: str, output: str, prev_ref: str, depth: int) -> Optional[Dict[str, str]]:
    """
    ...
    pyproject_toml = fetch_file(owner, repo_name, "pyproject.toml", current_ref, headers)
    ...
    if prev_ref_sha:
        try:
            prev_lock_content = fetch_file(owner, repo_name, "poetry.lock", prev_ref_sha, headers)
            prev_lock_data = toml.loads(prev_lock_content)
        except Exception as e:
            print(f"Warning: Could not fetch poetry.lock at previous release {latest_tag_name}: {e}")
...
def scan_dependencies(pyproject_data: dict, root_prev_lock: dict, root_curr_lock: dict, headers: Dict[str, str], depth: int, visited: Set[str]) -> Dict[str, list]:
    if depth <= 0:
        return {}

    deps = pyproject_data.get("tool", {}).get("poetry", {}).get("dependencies", {})
    git_deps = {name: info["git"] for name, info in deps.items() if isinstance(info, dict) and "git" in info}
    ...
    for dep_name, dep_git_url in git_deps.items():
        ...
        if old_sha:
            ...
            compare_url = f"https://api.github.com/repos/{dep_owner}/{dep_repo}/commits/{new_sha}"
        ...
        if tickets:
            release_notes[dep_name] = {
                "compare_url": f"https://github.com/{dep_owner}/{dep_repo}/compare/{old_sha}...{new_sha}" if old_sha else None,
        ...
        if depth > 1:
            try:
                dep_pyproject = fetch_file(dep_owner, dep_repo, "pyproject.toml", new_sha, headers)
...
def fetch_file(owner: str, repo: str, path: str, ref: str, headers: Dict[str, str]) -> str:
...
def get_latest_release_tag(owner: str, repo: str, headers: Dict[str, str]) -> Optional[str]:
...
def get_tag_commit_sha(owner: str, repo: str, tag_or_sha: str, headers: Dict[str, str]) -> str:
    import re
